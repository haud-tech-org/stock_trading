...
| `LOOKBACK_WINDOW` | 10 | The number of candles used for the trend confirmation logic. This value is also used as the **cooldown period** to prevent sequential alerts for the same event. |
...
4.  **Cooldown Mechanism**: Once an alert is generated at a specific index `i`, a cooldown period is activated. The algorithm will not generate another alert for the number of candles defined by `LOOKBACK_WINDOW`. This prevents a stream of duplicate alerts from a single trading event.

5.  **Alert Generation**: If all trend and re-alignment conditions are met and the cooldown is not active, an `AlertData` object is created with the corresponding `BUY` or `SELL` signal. In `DEPLOYMENT` mode, the function returns immediately with the first alert found.
...
graph TD
    subgraph "Backward Loop (for each candle 'i')"
        A[Start Loop at Latest Candle] --> B{Is candle in Cooldown Period?};
