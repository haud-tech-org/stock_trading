# Technical Reference: Deep-Dive Code Investigation - Architecture Findings

**Date:** April 8, 2026  
**Status:** Complete - Ready for Implementation Guides Architecture Documentation  
...
### 1.10 Data Layer Architecture ✅ (NEW COMPONENT - Foundation Layer)

**Location:** `src/stockreports/data_services/` (Complete data pipeline)  
**Documentation:** See `DATA_LAYER_ARCHITECTURE.md` for complete reference

**Core Responsibility:**
- Provide standardized OHLCV data to entire alert system
- Intelligent caching with miss detection
- Multi-provider coordination (Vietstock, Binance API, Binance CCXT)
- Data transformation (timezone conversion, price adjustments)

**7-Step Data Pipeline:**
```
[1] REQUEST DATA (from SymbolAlerter)
  ↓
[2] FETCH DATA VIA COORDINATOR (HistoricalDataManager)
  ↓
[3] EXTERNAL API CALLS (Provider routing)
  ↓
[4] RAW DATA PROCESSING (DataProcessor)
  ↓
[5] CACHE & RETURN RESULT (HistoricalDataManager)
  ↓
[6] DOWNSTREAM PROCESSING (Executors, PriceMovementAlerter)
  ↓
[7] FINAL RESULT (System output)
```

**Key Components:**
1. **HistoricalDataManager** - Central cache hub with intelligent fetching
2. **DataProviderCoordinator** - Multi-provider routing and selection
3. **DataProcessor** - Timezone conversion and price adjustments
4. **3 Providers** - Vietstock, Binance API, Binance CCXT

**Integration Point:**
- Called by: SymbolAlerter via DataServiceOrchestrator
- Method: `fetch_and_process(symbol, start_time, end_time, resolution)`
- Returns: Standardized pd.DataFrame with time index + OHLCV columns
- Used by: All Executors, PriceMovementAlerter (different resolutions per approach)

**Configuration:**
```python
# Single source of truth in src/stockreports/config/data_provider_settings.py
ENABLED_DATA_PROVIDERS = ["vietstock", "binance_ccxt"]
PROVIDER_SYMBOLS_CONFIG  # Maps symbols to providers
DATA_PROVIDER_CONFIG     # Timeout, retries, cache settings per provider
```

**Performance:**
- Cache lookup: <1ms (O(1) dict access)
- Provider creation: ~50ms (first time only, then cached)
- Single API fetch: 200-500ms (depends on external API)
- Full pipeline: 200-1500ms depending on cache hit/miss

**Design Patterns:**
- Factory + Registry: Provider creation and management
- Strategy: Provider-specific normalization
- Coordinator: Central orchestration
- Configuration-Driven: All behavior via settings
- Pipeline: Sequential data transformations

**Complete Reference:**
📖 See `TECHNICAL_REFERENCE/DATA_LAYER_ARCHITECTURE.md` for complete 7-step pipeline documentation, detailed component descriptions, design patterns, and API specifications.

---

### Component Summary: 10 Major Components

**Alert System Components (1-9):**
1. SymbolAlertManager - Multi-symbol orchestrator
2. SymbolAlerter - Single-symbol supervisor
3. ResolutionCoordinator - Approach-to-resolution mapping
4. DataServiceOrchestrator - Data fetching facade
5. PriceMovementAlerter - Price level detection
6. Executor Framework (6 implementations)
7. Close Position Scheduler - Time-based notifications
8. NotificationManager - Multi-channel dispatch
9. Supporting Components (Analyzer, Validator)

**Data Layer Component (10):**
10. Data Layer Architecture - Complete 7-step pipeline
    - HistoricalDataManager (cache hub)
    - DataProviderCoordinator (provider routing)
    - DataProcessor (transformations)
    - 3 Providers (Vietstock, Binance API, Binance CCXT)
...
## Technical Reference Complete ✅ (Updated with Time Sub-Modes)
...
**Ready for:** Implementation Guides - Create Unified Architecture Documentation

All findings verified through code inspection with exact file locations and code references.

---

## Implementation Guides Additions (Report Generation Layer)

⚠️ **NOTE:** Technical Reference documents the core alert and monitoring system. Implementation Guides introduces a new report generation layer for backtesting analysis:

**Implementation Guides Components (Not in Technical Reference scope):**
...
**Implementation Guides Configuration Additions:**
- **notification_settings.py** - Notification channel configuration (Implementation Guides)
- **validation_settings.py** - Profit/loss validation thresholds (Implementation Guides)

These components extend the base architecture for performance analysis and backtesting scenarios. See `PHASE1_PHASE2_ALIGNMENT.md` and Implementation Guides documentation (PERFORMANCE_METRICS_EXTENSION_GUIDE.md) for complete details.
