...
from src.stockreports.notification.unified_scheduler import check_and_notify
...
class SymbolAlerter:
    ...
    def _send_scheduled_notification(self, notifications):
        """
        Sends notifications that were generated by the scheduler.
        
        Args:
            notifications: List of AlertNotification objects (or single notification for backward compatibility)
        """
        # Handle both list (new unified scheduler) and single notification (backward compatibility)
        if isinstance(notifications, list):
            if notifications:
                self.logger.info(f"Scheduler: Sending {len(notifications)} scheduled notification(s)")
                for notification in notifications:
                    self.logger.debug(f"  - {notification.signal} for {notification.symbol}")
            ...
            # Backward compatibility: single notification
            self.logger.info(f"Scheduler: Sending scheduled notification: {notifications.signal}")
            self.notification_manager._send_alert(notifications)
        else:
            self.logger.debug("Scheduler: No scheduled notifications to send at this time.")
