...
class TestRealDataFetchingBinance:
    """Test real data fetching from Binance for crypto symbols."""
    ...
    def test_get_with_resolution_btcusdt_gmt7_time_range(self, manager):
        ...
        Validations:
        - Data response is not empty
        - Processed data (processed_df) is not empty
        - Display processed data in console
        """
        # Create a fresh manager instance
        test_manager = HistoricalDataManager()
        
        # GMT+7 is UTC+7, so:
        # 20:00:00 GMT+7 = 13:00:00 UTC
        # 21:00:00 GMT+7 = 14:00:00 UTC
        # Using 2024-01-15 as reference date
        
        start_time = pd.Timestamp('2024-01-15 13:00:00', tz='UTC')
        ...
        # Validation 1: Data response is not empty
        print(f"\n✓ Validation 1: Data Response Not Empty")
        assert result is not None, "Data response should not be None"
        assert not result.empty, "Data response should not be empty"
        print(f"  → Data retrieved successfully: {len(result)} candles")
        
        # Validation 2: Processed data (result from manager) is not empty
        print(f"\n✓ Validation 2: Processed Data Not Empty")
        assert len(result) > 0, "Processed data should contain rows"
        print(f"  → Processed data contains {len(result)} rows")
        ...
        print(f"  Timezone: {result.index.tz}")
        ...
        # Display statistics
        print(f"\nPrice Statistics:")
        print(f"  Open - Min: {result['open'].min():.2f}, Max: {result['open'].max():.2f}, Mean: {result['open'].mean():.2f}")
        print(f"  High - Min: {result['high'].min():.2f}, Max: {result['high'].max():.2f}, Mean: {result['high'].mean():.2f}")
        print(f"  Low  - Min: {result['low'].min():.2f}, Max: {result['low'].max():.2f}, Mean: {result['low'].mean():.2f}")
        print(f"  Close- Min: {result['close'].min():.2f}, Max: {result['close'].max():.2f}, Mean: {result['close'].mean():.2f}")
        
        # Display volume statistics
        print(f"\nVolume Statistics:")
        print(f"  Min: {result['volume'].min():.0f}")
        print(f"  Max: {result['volume'].max():.0f}")
        print(f"  Mean: {result['volume'].mean():.0f}")
        print(f"  Total: {result['volume'].sum():.0f}")
