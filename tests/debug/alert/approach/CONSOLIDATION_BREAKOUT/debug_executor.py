"""
A command-line tool for debugging the CONSOLIDATION_BREAKOUT alert logic by calling the main executor class.
...
def run_debug_analysis(symbol, start_time_str, end_time_str, save_to_file, generate_chart):
    ...
    executor = ConsolidationBreakoutExecutor(symbol=symbol)
    ...
    if generate_chart:
        if json_file_path:
            ...
            generate_visibility_chart(
                json_file_path, 
                chart_output_dir,
                approach_name="CONSOLIDATION_BREAKOUT",
                signal_type="BUY", 
                breakout_time_str="2025-11-25 11:27:00"
            )
        else:
...
if __name__ == "__main__":
    ...
    """
    parser = argparse.ArgumentParser(description="Debug the CONSOLIDATION_BREAKOUT logic by calling the main executor.")
